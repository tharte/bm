...
Just a reference (for Bibtex): \cite{Baxter-Rennie:96}.

* set-up                                                           :noexport:
** bash
#+begin_src bash :session *bash* :exports both :results output :eval no
root=/home/tharte/dot/projects/problems && cd $root
#+end_src 
...
- ~dd~ - for comparison
  ...
  + use ~if=/dev/urandom~ (whita's run with ~nohup~) to create ~o.bin~
